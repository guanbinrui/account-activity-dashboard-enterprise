    ...
    try {
        ...
        if (!response.ok) {
            const errorData = await response.json().catch(() => ({
                message: "Failed to parse user details error",
            }));
            console.error(
                `Error fetching user details for ${userId}: ${response.status}`,
                errorData
            );
        ...
        if (userDataResponse && userDataResponse.data) {
            ...
            console.warn(
                `User details not found or in unexpected format for ${userId}.`
            );
            return {
                id: userId,
                error: true,
                message: "User details not found or in unexpected format.",
            };
        }
    } catch (error) {
        ...
        return {
            id: userId,
            error: true,
            message: error.message || "Network error or similar",
        };
    }
}
...
function renderSubscriptionCards(webhookId, subscriptionsArray) {
    ...
    if (!subscriptionsArray || subscriptionsArray.length === 0) {
        container.innerHTML =
            "<p>No active subscriptions for this webhook.</p>";
        return;
    }
    ...
    subscriptionsArray.forEach((subscription) => {
        ...
        if (subscription.error) {
            ...
            const profileUrl = subscription.username
                ? `https://x.com/${subscription.username}`
                : "#";
...
async function fetchAndDisplaySubscriptions(webhookId, forceRefresh = false) {
    ...
    console.log(
        `[SubHandler] fetchAndDisplaySubscriptions for ${webhookId}, forceRefresh: ${forceRefresh}`
    );

    if (!forceRefresh && cachedUserSubscriptions[webhookId]) {
        console.log(
            `[SubHandler] Using cached subscriptions for webhook ${webhookId}`
        );
    ...
    try {
        const response = await fetch(
            `/api/webhooks/${webhookId}/subscriptions`
        );
        if (!response.ok) {
            const errorData = await response.json().catch(() => ({
                message: "Failed to parse error from server",
            }));
            throw new Error(
                `Error ${response.status}: ${errorData.error || errorData.details?.title || errorData.details?.detail || errorData.message || "Failed to fetch subscriptions"}`
            );
        }
        ...
        if (result.data && result.data.length > 0) {
            console.log(
                `[SubHandler] Fetched ${result.data.length} subscription IDs. Now fetching details...`
            );
            ...
            const userDetailPromises = result.data.map((subscriptionRef) =>
                fetchUserDetailsForSubscription(subscriptionRef.id).then(
                    (userData) => {
            ...
            console.log(
                "[SubHandler] All user details fetched.",
            ...
            console.log(
                `[SubHandler] No active subscriptions for webhook ${webhookId}.`
            );
        ...
        console.error(
            "[SubHandler] Failed to fetch or display subscriptions:",
            err
        );
...
function confirmDeleteSubscription(webhookId, userId) {
    if (
        confirm(
            `Are you sure you want to delete the subscription for User ID: ${userId} from webhook ${webhookId}?`
        )
    ) {
...
async function handleDeleteSubscription(webhookId, userId) {
    ...
    try {
        const response = await fetch(
            `/api/webhooks/${webhookId}/subscriptions/${userId}`,
            {
                method: "DELETE",
            }
        );
        ...
        if (response.ok) {
            ...
            if (
                ...
                errorData.details.errors[0].message
            ) {
...
async function handleAddSubscription() {
    const webhookSelectElement = document.getElementById(
    ...
    if (!webhookSelectElement || !messageElement) {
        console.error(
            "Required elements for adding subscription not found in subscriptionHandler.js."
        );
        if (messageElement) {
            messageElement.textContent =
                "Error: UI elements missing. Please refresh.";
    ...
    try {
        const response = await fetch(
            `/api/webhooks/${selectedWebhookId}/subscriptions`,
            {
                method: "POST",
                headers: {},
            }
        ...
        if (response.ok) {
            messageElement.textContent =
                responseData.message || "Subscription request successful!";
            ...
            if (typeof fetchAndDisplaySubscriptions === "function") {
                ...
                console.error(
                    "fetchAndDisplaySubscriptions function not found to refresh list."
                );
            }
        } else {
            ...
            if (responseData) {
                if (
                    ...
                    responseData.details.errors.length > 0 &&
                    responseData.details.errors[0].message
                ) {
                    detailedErrorMessage =
                        responseData.details.errors[0].message;
                } else if (
                    responseData.details &&
                    responseData.details.detail
                ) {

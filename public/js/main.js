...
document.addEventListener("DOMContentLoaded", () => {
    ...
    document
        .querySelectorAll(".navbar a[data-tab]")
        .forEach((tabLink, index) => {
            console.log(
                `[MainJS_Debug] Attaching click listener to: ${tabLink.getAttribute("data-tab")}`
            );
    ...
    const refreshSubscriptionsBtn = document.getElementById(
    ...
    if (refreshSubscriptionsBtn) {
        refreshSubscriptionsBtn.addEventListener("click", () => {
            ...
            if (
                lastSelectedWebhookIdForSubscriptions &&
                typeof window.fetchAndDisplaySubscriptions === "function"
            ) {
                window.fetchAndDisplaySubscriptions(
                    lastSelectedWebhookIdForSubscriptions,
                    true
                );
            } else {
                console.warn(
                    "[MainJS_Debug] No webhook selected or fetchAndDisplaySubscriptions not available for refresh."
                );
            }
        });
    }
    ...
    console.log(
        "[MainJS_Debug] About to call handleHashChange for initial load."
    );
    handleHashChange();
...
function handleHashChange() {
    ...
    console.log(
        `[MainJS] Hash change or initial load. Attempting to show tab: ${tabToOpen}`
    );
    showTab(tabToOpen);
}
...
function showTab(tabId) {
    console.log(
        `[MainJS_Debug] showTab function entered for tabId: ${tabId}. Current open tab: ${currentOpenTab}`
    );
    ...
    if (currentOpenTab === "live-events" && tabId !== "live-events") {
        if (typeof window.closeLiveEventsConnection === "function") {
            console.log(
                "[MainJS_Debug] Navigating away from LiveEvents. Closing WebSocket connection."
            );
    ...
    if (tabId === "webhooks") {
        ...
        if (typeof initializeSubscriptionsPage === "function")
            initializeSubscriptionsPage();
    } else if (tabId === "live-events") {
        if (typeof window.initializeLiveEvents === "function") {
            console.log(
                "[MainJS_Debug] LiveEvents tab is active. Initializing WebSocket connection."
            );
            window.initializeLiveEvents();
        } else {
            console.error(
                "[MainJS_Debug] initializeLiveEvents function not found!"
            );
        }
    }
}
...
function renderWebhooksList(webhooksArray) {
    ...
    if (!listElement || !errorElement) {
        console.error(
            "[MainJS] Webhooks list or error element not found for rendering."
        );
        return;
    }
    ...
    } else if (webhooksArray) {
        // webhooksArray exists but is empty
        listElement.innerHTML = "<li>No webhooks registered.</li>";
    } else {
        // webhooksArray is null or undefined (should ideally not happen if called correctly)
...
async function fetchWebhooks(forceRefresh = false) {
    ...
    console.log(
        `[MainJS_Debug] fetchWebhooks called. Force refresh: ${forceRefresh}`
    );
    ...
    try {
        ...
        if (!response.ok) {
            ...
            throw new Error(
                `Error ${response.status}: ${errorData.error || errorData.details?.title || errorData.details?.detail || errorData.message || "Failed to fetch webhooks"}`
            );
        }
        ...
        console.log(
            "[MainJS_Debug] Webhooks fetched and cached:",
            cachedWebhooksData
        );
        renderWebhooksList(cachedWebhooksData);
    } catch (err) {
            ...
            if (typeof twitterError === "object" && twitterError !== null) {
                if (
                    ...
                    detailedErrorMessage =
                        twitterError.title +
                        (twitterError.detail ? `: ${twitterError.detail}` : "");
...
async function confirmDeleteWebhook(webhookId, webhookUrl) {
    if (
        !confirm(
            `Are you sure you want to delete the webhook for URL: ${webhookUrl} (ID: ${webhookId})?`
        )
    ) {
        return;
    }
    ...
    try {
        ...
        if (response.ok) {
            ...
            const errorData = await response.json().catch(() => ({
                message:
                    "Failed to parse server error response for delete operation",
            }));
            ...
            if (errorData.details) {
                ...
                if (typeof twitterError === "object" && twitterError !== null) {
                    if (
                        ...
                        finalDetailMessage = JSON.stringify(twitterError);
                    }
                } else if (typeof twitterError === "string") {
                    finalDetailMessage = twitterError;
                }
...
async function populateWebhookDropdownForSubscriptions() {
    ...
    try {
        ...
        if (!response.ok) {
            const errorData = await response.json().catch(() => ({
                message: "Failed to parse webhook list for dropdown",
            }));
            throw new Error(
                `Error ${response.status}: ${errorData.error || errorData.details?.title || errorData.details?.detail || errorData.message || "Failed to fetch webhooks for dropdown"}`
            );
        }
        const result = await response.json();
        ...
        if (result.data && result.data.length > 0) {
            ...
            noWebhooksOption.textContent =
                "No webhooks registered. Please add one first.";
            selectElement.appendChild(noWebhooksOption);
        }
    } catch (err) {
        console.error(
            "Failed to populate webhook dropdown for subscriptions:",
            err
        );
        errorElement.textContent =
            err.message ||
        ...
        selectElement.innerHTML =
            '<option value="">Error loading webhooks</option>';
    } finally {
...
function handleSubscriptionWebhookChange(event) {
    ...
    const subscriptionsListContainer = document.getElementById(
    ...
    if (selectedWebhookId) {
        ...
        if (typeof fetchAndDisplaySubscriptions === "function") {
            ...
            console.error(
                "fetchAndDisplaySubscriptions is not defined. Ensure subscriptionHandler.js is loaded."
            );
            if (subscriptionsListContainer)
                subscriptionsListContainer.innerHTML =
                    '<p style="color:red">Error: UI Component not loaded.</p>';
        }
    } else {
        subscriptionsListContainer.innerHTML =
...
async function initializeSubscriptionsPage() {
    ...
    if (lastSelectedWebhookIdForSubscriptions) {
        const optionExists = selectElement.querySelector(
            `option[value="${lastSelectedWebhookIdForSubscriptions}"]`
        );
        if (optionExists) {
            console.log(
                `[MainJS_Debug] Restoring last selected webhook for subscriptions: ${lastSelectedWebhookIdForSubscriptions}`
            );
            selectElement.value = lastSelectedWebhookIdForSubscriptions;
        } else {
            console.log(
                `[MainJS_Debug] Last selected webhook ${lastSelectedWebhookIdForSubscriptions} not found in dropdown, resetting.`
            );
...
async function handleConfirmReplay() {
    ...
    if (!fromDateInput || !toDateInput || !messageElement) {
        console.error(
            "[MainJS] Replay modal date/time input or message element not found."
        );
    ...
    if (toDateObj.getTime() - fromDateObj.getTime() < 60000) {
    ...
    try {
        ...
        if (response.status === 200) {
            ...
            const errorData = await response.json().catch(() => ({
                message: "Failed to parse error response for replay",
            }));
...
function showValidateWebhookModal(webhookId) {
    ...
    console.log(
        `[MainJS] Show validate webhook modal for webhook ID: ${webhookId}`
    );
}
...
async function handleConfirmValidateWebhook() {
    ...
    statusContainer.innerHTML =
        '<div class="modal-spinner"></div> Validating...'; // Placeholder for animation
    confirmButton.disabled = true;

    try {
        const response = await fetch(
            `/api/webhooks/${currentValidateWebhookId}`,
            {
                method: "PUT",
            }
        );
        ...
        if (response.status === 204) {
            ...
            const errorData = await response.json().catch(() => ({
                message: "Failed to parse server error response for validation",
            }));

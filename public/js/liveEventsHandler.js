...
function createEventCard(eventData) {
    ...
    if (eventData.type === "connection_ack") {
        ...
        eventData.tweet_create_events &&
        eventData.tweet_create_events.length > 0
    ) {
        ...
        eventData.tweet_delete_events &&
        eventData.tweet_delete_events.length > 0
    ) {
        ...
        eventData.favorite_events &&
        eventData.favorite_events.length > 0
    ) {
        ...
        const actionText =
            followEvent.type === "follow" ? "followed" : "unfollowed";
        ...
        eventSpecificClass =
            muteEvent.type === "mute" ? "event-card-mute" : "event-card-unmute";
        ...
        eventData.direct_message_events &&
        eventData.direct_message_events.length > 0
    ) {
        ...
        eventData.direct_message_indicate_typing_events &&
        eventData.direct_message_indicate_typing_events.length > 0
    ) {
        ...
        if (recipientId === forUserId) {
            // Typing event is directed at the subscribed user
            primaryContent = `<p><em>@${sender.screen_name} (${sender.name}) is typing...</em></p>`;
        } else {
            // Should not happen if webhook is for forUserId, but good to be complete
        ...
        eventData.direct_message_mark_read_events &&
        eventData.direct_message_mark_read_events.length > 0
    ) {
        ...
        const forUserId = eventData.for_user_id;
        const users = eventData.users || {};
        const reader = users[readerId] || {
            screen_name: readerId,
            name: "Unknown User",
        };
        const originalSender = users[originalSenderId] || {
            screen_name: originalSenderId,
            name: "Unknown User",
        };
        ...
        if (originalSenderId === forUserId) {
            ...
            // This case might be less common if webhooks are configured for a specific user to monitor *their* events.
...
function initializeLiveEvents() {
    const liveEventsContainer = document.getElementById(
    ...
    if (liveEventsSocket && liveEventsSocket.readyState === WebSocket.OPEN) {
        ...
        addEventToContainer({
            type: "system_message",
            message: "Re-focused tab. WebSocket was already open.",
    ...
    liveEventsSocket.onopen = function (event) {
        ...
        if (
            liveEventsContainer.firstChild &&
            liveEventsContainer.firstChild.tagName === "P" &&
            liveEventsContainer.firstChild.textContent.includes(
    ...
    liveEventsSocket.onerror = function (event) {
        ...
        addEventToContainer({
            type: "system_error",
            message: "WebSocket error observed. See console for details.",
        });
        if (
            liveEventsContainer.firstChild &&
            liveEventsContainer.firstChild.tagName === "P" &&
            ...
            liveEventsContainer.innerHTML =
                '<p style="color:red;"><i>Error connecting to live event stream. See console.</i></p>';
        }
    };

    liveEventsSocket.onclose = function (event) {
        console.log(
            "[WebSocket] Connection closed for live events. Code:",
            event.code,
            "Reason:",
            event.reason
        );
        ...
        if (
            ...
            liveEventsContainer.innerHTML =
                '<p style="color:orange;"><i>Disconnected from live event stream. Will attempt to reconnect if you revisit this tab.</i></p>';
        }
    };
}

...
- Copy the environment template file:
    ...
    cp env.template .env
    ```
- Edit the `.env` file and fill in your X API credentials and dashboard credentials:
...
**Note:** If `DASHBOARD_USERNAME` or `DASHBOARD_PASSWORD` are not set, both default to `admin`. For security, always set strong credentials in production.
...
You will be redirected to the login page. Enter your dashboard credentials (set via `DASHBOARD_USERNAME` and `DASHBOARD_PASSWORD` in `.env`, or both default to `admin` if not set) to access the dashboard.

After successful authentication, you will see the Account Activity Dashboard interface. Your session will remain active for 24 hours.
...
4.  **Construct your Webhook URL:** Your full webhook URL for X will be the ngrok HTTPS URL followed by `/webhooks/twitter`.
    _Example:_ `https://your-unique-id.ngrok-free.app/webhooks/twitter`

...
function jsonResponse(
    ...
    console.log(
        `[RESPONSE] ${method} ${pathname} - Status: ${status}, Body: ${bodyStr}`
    );
...
function noContentResponse(
    ...
    console.log(
        `[RESPONSE] ${method} ${pathname} - Status: ${status}, Body: (empty)`
    );
...
async function getWebhooks(req: Request, url: URL): Promise<Response> {
    ...
    try {
        ...
        if (!response.ok) {
            console.error(
                `Twitter API GET Error: ${response.status} ${response.statusText}`,
                data
            );
            return jsonResponse(
                response.status,
        ...
        console.error(
            "Error fetching from Twitter API (GET /webhooks):",
            error
        );
        return jsonResponse(
            500,
            { error: "Internal server error while fetching from Twitter API." },
            req.method,
            url.pathname
        );
    }
...
async function createWebhook(req: Request, url: URL): Promise<Response> {
    ...
    try {
        ...
        if (!response.ok) {
            console.error(
                `Twitter API POST Error: ${response.status}`,
                responseData
            );
        ...
        let errorBody = {
            error: "Internal server error while creating webhook.",
        };
        let errorStatus = 500;
        if (
        ...
        console.error(
            "Error creating webhook via Twitter API (POST /webhooks):",
            error
        );
    ...
    try {
        ...
        if (!response.ok) {
            ...
            try {
                ...
                errorDetails =
                    twitterErrorData.title ||
                    twitterErrorData.detail ||
                    JSON.stringify(twitterErrorData);
            ...
            console.error(
                `Twitter API PUT Error: ${response.status}`,
                errorDetails
            );
            return jsonResponse(
                response.status,
        ...
        console.error(
            "Error sending validation request via Twitter API (PUT /webhooks/:id):",
            error
        );
        return jsonResponse(
            500,
            {
                error: "Internal server error while sending validation request.",
            },
            req.method,
            url.pathname
        );
...
async function deleteWebhook(
    ...
    try {
        ...
        if (!response.ok) {
            ...
            console.error(
                `Twitter API DELETE Error: ${response.status}`,
                errorDetails
            );
            return jsonResponse(
                response.status,
        ...
        console.error(
            "Error deleting webhook via Twitter API (DELETE /webhooks/:id):",
            error
        );
        return jsonResponse(
            500,
            { error: "Internal server error while deleting webhook." },
            req.method,
            url.pathname
        );
    }
...
async function replayWebhookEvents(
    req: Request,
    url: URL,
    webhookId: string
): Promise<Response> {
    ...
    if (!bearerToken) {
        console.error(
            `X_BEARER_TOKEN not found for POST /api/webhooks/${webhookId}/replay.`
        );
        return jsonResponse(
            500,
            { error: "Server configuration error: Missing API token." },
    ...
    try {
        ...
        console.log(
            `[DEBUG] Sending POST to X API for replay: ${twitterApiUrl} (UTC times) from local inputs: from=${from_date}, to=${to_date}`
        );
        ...
        if (response.status === 200) {
            // OK
            const responseData =
                (await response.json()) as XReplaySuccessResponse;
            console.log(
                `[DEBUG] X API Replay request successful for webhook ${webhookId}. Job ID: ${responseData?.data?.job_id}`
            );
        ...
        console.error(
            "Error processing replay request (POST /api/webhooks/:id/replay):",
            error
        );
        return jsonResponse(
            500,
            { error: "Internal server error while requesting event replay." },
            req.method,
            url.pathname
        );
    }
...
export async function handleWebhookRoutes(
    ...
    if (req.method === "GET" && !primaryId) {
        // GET /api/webhooks
        return getWebhooks(req, url);
    }
    if (req.method === "POST" && !primaryId) {
        // POST /api/webhooks
        return createWebhook(req, url);
    }

    if (primaryId && !secondaryAction) {
        // Routes like /api/webhooks/:id for PUT, DELETE
        if (req.method === "PUT") {
    ...
    const replayMatch = url.pathname.match(
        /^\/api\/webhooks\/([a-zA-Z0-9_]+)\/replay$/
    );
    if (replayMatch && req.method === "POST") {
        ...
        console.error(
            "[ROUTE_ERROR] Webhook ID not found in replay path despite match.",
            url.pathname
        );
        return jsonResponse(
            400,
            { error: "Invalid webhook ID in path for replay." },
            req.method,
            url.pathname
        );
    }

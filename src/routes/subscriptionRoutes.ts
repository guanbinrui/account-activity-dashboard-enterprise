...
function jsonResponse(
    ...
    console.log(
        `[SUBSCRIPTION_RESPONSE] ${method} ${pathname} - Status: ${status}, Body: ${bodyStr}`
    );
...
function noContentResponse(method: string, pathname: string): Response {
    console.log(
        `[SUBSCRIPTION_RESPONSE] ${method} ${pathname} - Status: 204 (No Content)`
    );
...
async function getWebhookSubscriptions(
    ...
    if (!bearerToken) {
        console.error(
            `X_BEARER_TOKEN not found for GET /api/webhooks/${webhookId}/subscriptions.`
        );
        return jsonResponse(
            500,
    ...
    try {
        ...
        console.log(
            `[DEBUG] Fetching subscriptions from Twitter API: ${twitterApiUrl}`
        );
        ...
        if (!response.ok) {
            console.error(
                `Twitter API GET Subscriptions Error: ${response.status} ${response.statusText}`,
                twitterResponseData
            );
            return jsonResponse(
                response.status,
        ...
        if (
            twitterResponseData &&
            twitterResponseData.data &&
            Array.isArray(twitterResponseData.data.subscriptions)
        ) {
            subscriptions = twitterResponseData.data.subscriptions.map(
                (sub: { user_id: string }) => ({ id: sub.user_id })
            );
        } else {
            console.warn(
                "Twitter API GET Subscriptions: Response structure was not as expected or no subscriptions array.",
                twitterResponseData
            );
        }
        ...
        return jsonResponse(
            ...
            {
                data: subscriptions,
                meta: { result_count: subscriptions.length },
            },
            req.method,
            url.pathname
        );
        ...
        console.error(
            `Error fetching subscriptions for webhook ${webhookId} from Twitter API:`,
            error
        );
        return jsonResponse(
            500,
...
async function createWebhookSubscription(
    ...
    if (!consumerKey || !consumerSecret || !accessToken || !accessTokenSecret) {
        console.error(
            "Missing OAuth 1.0a credentials in environment variables for subscriptions."
        );
        return jsonResponse(
            500,
            {
    ...
    console.log(
        `[DEBUG] Attempting to add subscription with OAuth 1.0a. URL: ${twitterApiUrl}`
    );
    ...
    try {
        ...
        if (!response.ok) {
            console.error(
                `Twitter API POST Subscriptions/all Error: ${response.status} ${response.statusText}`,
                responseData
            );
            return jsonResponse(
                response.status,
        ...
        if (response.status === 204) {
            ...
            return jsonResponse(
                204,
                { message: "Subscription successful (or already exists)." },
                req.method,
                url.pathname
            );
        }
        if (response.status === 200 && responseData) {
            console.log(
                "Twitter API POST Subscriptions/all successful with 200:",
                responseData
            );
        ...
        console.warn(
            "Twitter API POST Subscriptions/all: Unexpected successful response status or format",
            { status: response.status, body: responseData }
        );
        return jsonResponse(
            response.status,
            {
                message:
                    "Subscription request processed, but response format unexpected.",
                details: responseData,
            },
            req.method,
            url.pathname
        ...
        console.error(
            `Error adding subscription for webhook ${webhookId} (OAuth 1.0a):`,
            error
        );
        return jsonResponse(
            500,
            { error: "Internal server error while adding subscription." },
            req.method,
            url.pathname
        );
    }
...
async function deleteWebhookSubscription(
    ...
    if (!bearerToken) {
        console.error(
            `X_BEARER_TOKEN not found for DELETE /api/webhooks/${webhookId}/subscriptions/${userId}.`
        );
        return jsonResponse(
            500,
            { error: "Server configuration error: Missing API token." },
    ...
    console.log(
        `[DEBUG] Attempting to delete subscription. User ID: ${userId}, Webhook ID: ${webhookId}. URL: ${twitterApiUrl}`
    );
    ...
    try {
        ...
        if (response.status === 204) {
            // Twitter typically returns 204 No Content on successful deletion
        ...
        const responseData = await response.json().catch(() => ({
            message:
                "Failed to parse error response from Twitter API or no content.",
        }));
        console.error(
            `Twitter API DELETE Subscription Error: ${response.status} ${response.statusText}`,
            responseData
        );
        return jsonResponse(
            response.status,
        ...
        console.error(
            `Error deleting subscription for User ID ${userId} from webhook ${webhookId}:`,
            error
        );
        return jsonResponse(
            500,
...
export async function handleSubscriptionRoutes(
    ...
    const deleteMatch = fullPath.match(
        new RegExp(`^${subscriptionsBasePath}/([^/]+)$`)
    ); // Capture userId

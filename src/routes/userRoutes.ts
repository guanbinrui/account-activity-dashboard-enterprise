...
function jsonResponse(
    ...
    console.log(
        `[USER_API_RESPONSE] ${method} ${pathname} - Status: ${status}, Body: ${bodyStr}`
    );
...
async function getUserDetails(
    ...
    console.log(
        `[DEBUG] Fetching user details from Twitter API: ${twitterApiUrl}`
    );
    ...
    try {
        ...
        if (!response.ok) {
            console.error(
                `Twitter API GET User Details Error for ID ${userId}: ${response.status} ${response.statusText}`,
                responseData
            );
            return jsonResponse(
                response.status,
        ...
        console.error(
            `Error fetching user details for ID ${userId} from Twitter API:`,
            error
        );
        return jsonResponse(
            500,
